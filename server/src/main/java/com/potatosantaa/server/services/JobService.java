...
public class JobService {
    ...
    public String addJob(JobApp job) throws InterruptedException, ExecutionException, FirebaseAuthException {
        ...
        ApiFuture<WriteResult> writeResult = db.collection("Users/" + getUser().getUID() + COL_NAME)
                .document(job.getJobID()).set(job);
    ...
    public String deleteJob(String jobId) throws FirebaseAuthException {
        Firestore db = FirestoreClient.getFirestore();
        ApiFuture<WriteResult> writeResult = db.collection("Users/" + getUser().getUID() + COL_NAME).document(jobId)
                .delete();
